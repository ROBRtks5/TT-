...
export class BotKernel {
      ...
      if (!this.disposed) {
        ...
        if (this.state.isBotActive && status === "STALE_DATA") {
          if (
            ...
            now - this.staleStatusStartTime > 45000
          ) {
            this.log(
              LogType.WARNING,
              "🔥 PHOENIX: Данные отсутствуют слишком долго (45с+). Принудительная инициация восстановления...",
            );
        ...
        if (
          ...
          this.state.status === BotStatus.TRADING &&
          !this.isSynchronizing
        ) {
          this.syncWatchdogTicks++;
          if (this.syncWatchdogTicks >= 30) {
            this.syncWatchdogTicks = 0;
            this.log(
              LogType.SYSTEM,
    ...
    this.uiUpdateTimer = setInterval(() => {
      if (this.pendingStateUpdate && !this.disposed) {
        this.postMessage({
          ...
          payload: this.pendingStateUpdate,
        });
    ...
    try {
      switch (command.type) {
        ...
        case "UPDATE_BIOS_SETTINGS":
          this.updateState({
            systemConfig: { ...this.state.systemConfig, ...command.payload },
          });
        ...
        case "SET_INSTRUMENT_TICKER":
          ...
          this.log(
            LogType.SYSTEM,
            `Целевой инструмент изменен на: ${command.payload}`,
          );
          // HOT RELOAD LOGIC
          if (this.state.isBotActive) {
            this.log(
              LogType.INFO,
              "🔄 Перезагрузка данных для нового инструмента...",
            );
            this.dataController
              .initialize()
              .then(() => {
              ...
              .catch((err) => {
                this.log(
                  LogType.ERROR,
                  "Ошибка смены инструмента: " + err.message,
                );
                this.updateState({
                  isBotActive: false,
  ...
  private async runSimulation() {
    this.log(
      LogType.INFO,
      "🧪 SIMULATION: Модуль симуляции отключен в текущей версии.",
    );
  }
  ...
  public async syncStateWithExchange(isInitial: boolean = false) {
    ...
    this.log(
      LogType.SYSTEM,
      `🔄 СИНХРОНИЗАЦИЯ: ${isInitial ? "Стартовая" : "Runtime"} сверка данных...`,
    );
    ...
    try {
      ...
      if (isInitial) {
        ...
        if (exchangeOrders.length > 0 || lqdtOrders.length > 0) {
          ...
          while (attempts < 5) {
            ...
            this.log(
              LogType.INFO,
              `⏳ Ожидание снятия ордеров (попытка ${attempts + 1}/5)...`,
            );
            attempts++;
          }
        ...
        const uniqueOrders = Array.from(
          new Map(consolidatedOrders.map((o) => [o.orderId, o])).values(),
        );
        this.updateState({ activeGridOrders: uniqueOrders });
        this.log(
          LogType.INFO,
          `♻️ Синхронизация завершена: ${uniqueOrders.length} ордеров в памяти.`,
        );
      }
      ...
      if (realPosition && Math.abs(realPosition.currentQuantity) > 0) {
        this.log(
          LogType.INFO,
          `📊 Обнаружена позиция: ${realPosition.currentQuantity} шт @ ${realPosition.entryPrice.toFixed(2)}`,
        );
  ...
  private async start() {
    if (this.isSystemStarting || this.state.status === BotStatus.TRADING)
      return;
