...
const Dashboard: React.FC = () => {
  ...
  const latestPrice = useMemo(() => {
    if (bot.lastTrades && bot.lastTrades.length > 0)
      return bot.lastTrades[0].price;
    if (
      bot.chartData &&
      bot.chartData["1m"] &&
      bot.chartData["1m"].length > 0
    ) {
  ...
  const handleSaveApiKey = useCallback(
    async (apiKey: string, geminiKey?: string) => {
  ...
  return (
    <div className="titan-grid bg-black text-gray-200 font-mono relative select-none overflow-hidden h-[100dvh] flex flex-col">
      ...
      >
        Внимание: Позиция останется открытой. Робот перестанет управлять
        ордерами.
      </ConfirmationDialog>
      <ConfirmationDialog
      ...
      <main className="flex-1 flex flex-col min-h-0 relative z-10">
        ...
        <div className="flex-1 flex min-h-0">
          <WidgetBoundary
            label="THE_GRID"
            className="flex-[2] border-r border-gray-900 relative"
          >
            <TradeGrid
          ...
          <WidgetBoundary
            label="INTELLIGENCE"
            className="flex-1 min-w-[300px] bg-black hidden lg:block border-l border-gray-800"
          >
