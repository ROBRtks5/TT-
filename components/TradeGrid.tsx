...
import { Position, BotStatus, GridOrder, TradeDirection } from '../types';
...
const TradeGrid: React.FC<TradeGridProps> = ({ 
    position, lastPrice = 0, ammCapitalState, status, activeGridOrders = []
}) => {
    ...
    return (
        <div className="w-full h-full flex flex-col relative px-4 py-4 bg-black overflow-hidden font-mono text-gray-300">
            ...
            {/* Top Balances Display - Compact */}
            <div className="w-full flex justify-between z-20 mb-2 gap-2">
            ...
            <div className="flex-1 overflow-y-auto z-10 flex flex-col w-full pr-1">
                ...
                <div className="flex flex-col gap-1 w-full mt-auto">
                    {sells.map((order, idx) => (
                        <div key={order.orderId} className="flex justify-between items-center py-1 px-2 border-l-2 border-cyber-pink bg-cyber-pink/5 hover:bg-cyber-pink/10 transition-colors text-xs sm:text-sm">
                ...
                <div className="flex flex-col gap-1 w-full mb-auto">
                    {buys.map((order, idx) => (
                        <div key={order.orderId} className="flex justify-between items-center py-1 px-2 border-l-2 border-cyber-cyan bg-cyber-cyan/5 hover:bg-cyber-cyan/10 transition-colors text-xs sm:text-sm">
