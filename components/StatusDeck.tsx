...
const StatusDeck: React.FC<StatusDeckProps> = ({ 
    ...
    useEffect(() => {
        const updateTime = () => {
            try {
                ...
                setClock(`${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`);
    ...
    // Sensor Logic
    const netStatus: SensorStatus = connectionStatus === 'CONNECTED' ? 'OK' : connectionStatus === 'CONNECTING' || connectionStatus === 'RECONNECTING' ? 'WARNING' : 'ERROR';
    ...
    const handleDebugDump = () => {
        ...
        const context = {
            ...
            history: {
                ...
                lastPnlResult: botState?.tradeHistory?.[botState?.tradeHistory?.length - 1]?.pnl,
                recentTrades: botState?.tradeHistory?.slice(-5) || []
            },
            marketData: {
                lastTrades: botState?.lastTrades?.slice(0, 5) || [],
                lastCandles: typeof botState?.chartData?.['1m'] !== 'undefined' ? botState.chartData['1m'].slice(-3) : [],
    ...
    return (
        <div className="w-full h-full flex items-center justify-between px-3 bg-black border-b border-gray-800 z-50 relative shadow-md">
            ...
            <div className="flex flex-col gap-1.5 min-w-[70px]">
                ...
                <SensorLED label={coreLabel} status={kernelStatus} showGlow={status !== BotStatus.MARKET_CLOSED} />
            </div>
            ...
            >
                ...
                <div className={`text-[9px] font-bold tracking-[0.2em] uppercase mt-1 ${sessionInfo.code === 'OPEN' ? 'text-cyber-green' : 'text-gray-600'}`}>
                    {sessionTimer}
                </div>
                ...
                {showSchedule && (
                    <div className="absolute top-14 left-1/2 -translate-x-1/2 w-48 bg-black/95 border border-gray-700 rounded p-3 shadow-2xl backdrop-blur-md z-[100] animate-fade-in-up">
                        <div className="text-[10px] text-gray-500 uppercase font-bold border-b border-gray-800 pb-1 mb-2">Статус Биржи (MSK)</div>
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-gray-400 text-xs">Режим:</span>
                            <span className={`text-xs font-bold ${sessionInfo.code === 'OPEN' ? 'text-green-400' : 'text-orange-400'}`}>
                                {sessionInfo.text}
                            </span>
                        </div>
                    </div>
            ...
            {/* RIGHT: COMMANDS (3 Distinct Buttons) */}
