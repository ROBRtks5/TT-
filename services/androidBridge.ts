...
export const setWakeLock = async (isActive: boolean) => {
    ...
    try {
        if (isActive) {
            ...
            console.debug('[AndroidBridge] WAKE_LOCK enabled (Bot Active).');
        } else {
            await KeepAwake.allowSleep();
            console.debug('[AndroidBridge] WAKE_LOCK disabled (Bot Idle).');
        }
    } catch (e) {
...
export const initializeAndroidBridge = async () => {
    if (!Capacitor.isNativePlatform()) {
        console.debug('[AndroidBridge] Running in web mode. Skipping native Android initialization.');
        return;
    }

    try {
        console.debug('[AndroidBridge] Native platform detected. Initializing specific protocols...');

        // 1. Keep Awake (Wake Lock)
        // This stops the screen from dimming/sleeping when the app is in the foreground.
        await KeepAwake.keepAwake();
        console.debug('[AndroidBridge] WAKE_LOCK acquired successfully.');
        ...
        App.addListener('appStateChange', ({ isActive }) => {
            console.debug(`[AndroidBridge] App state changed. isActive: ${isActive}`);
            if (!isActive) {
                // The app has gone to the background. 
                console.debug('[AndroidBridge] App sent to background. Core engine continues running if allowed by OS.');
            } else {
                console.debug('[AndroidBridge] App returned to foreground.');
            }
        });

        // 3. Optional: Back Button Override (prevent accidental exit)
        App.addListener('backButton', ({ canGoBack }) => {
            console.debug('[AndroidBridge] Back button pressed. Triggering custom exit modal.');
        ...
        console.debug('[AndroidBridge] Initialization complete.');
    } catch (error) {
